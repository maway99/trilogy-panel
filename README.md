...
| Chrome kiosk     | MA2 PC              | Task Scheduler at logon, 60s grace delay   |
...
This installs dependencies, builds the client, registers PM2 + the Chrome kiosk in Task Scheduler, and saves the PM2 process list. Reboot once afterwards to verify the full auto-start chain.

Prerequisites (install manually before running setup.ps1):
...
**Normal start:** PC boots → MA2 launches → PM2 brings the server up → 60s later Chrome opens in kiosk mode → panel shows ✅ MA2 + ✅ Resolume in the sidebar.
...
| Panel server offline overlay                     | `pm2 status`. If `trilogy-panel` is stopped: `pm2 start ecosystem.config.js`. If it's looping crashes, `pm2 logs trilogy-panel` will show why. |
| Chrome didn't auto-launch on boot                | `Task Scheduler → Trilogy Chrome Kiosk → Last Run Result`. The .bat has a 60s delay — give it time. |
